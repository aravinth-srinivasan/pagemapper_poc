package com.raweng.pagemapper.pagemappersdk.views.gamestats.provider
...
import com.raweng.pagemapper.pagemappersdk.views.gamestats.mapper.GameStatsMapper
...
class GameStatsDataProvider(
    ...
    private fun getResponseDataModel(data: Pair<GameStatsCardResponse?, GameStatsResponseAndStateModel>): ResponseDataModel {
        val response = ResponseDataModel(item = item, apiResponse = data.second, convertedData = null)
        val dfeResponse = data.second
        val isLiveGame = ((dfeResponse.gameLeader.isNotEmpty()) && (gameId.isNotEmpty()))
    ...
    private suspend fun getCMSData(): GameStatsCardResponse? {
        ...
        val cmsUseCase =
            CMSBaseUseCase(item, cmsRepository, GameStatsCardResponse::class.java)
    ...
    private suspend fun getDFEData(): GameStatsResponseAndStateModel {
        ...
        val dfeResponse = dfeUseCase.execute(request, gameId)
